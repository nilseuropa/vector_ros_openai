...
import rospy
import time
from vector_ros_openai import robot_gazebo_env
...
class AnkiVectorEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        * /vector/laser: Range Readings

        Actuators Topic List: /cmd_vel,

        Args:
        """
        rospy.logdebug("Start AnkiVectorEnv INIT...")
        ...
        super(AnkiVectorEnv, self).__init__(controllers_list=self.controllers_list,
        ...
        rospy.Subscriber("/vector/imu", Imu, self._imu_callback)
        rospy.Subscriber("/vector/laser", Range, self._laser_scan_callback)
        ...
        rospy.logdebug("Finished AnkiVectorEnv INIT...")
    ...
    def _check_imu_ready(self):
        self.imu = None
        rospy.logdebug("Waiting for /vector/imu to be READY...")
        while self.imu is None and not rospy.is_shutdown():
            try:
                self.imu = rospy.wait_for_message("/vector/imu", Imu, timeout=5.0)
                rospy.logdebug("Current /vector/imu READY=>")

            except:
                rospy.logerr("Current /vector/imu not ready yet, retrying for getting imu")
    ...
    def _check_laser_scan_ready(self):
        ...
        rospy.logdebug("Waiting for /vector/laser to be READY...")
        while self.laser_scan is None and not rospy.is_shutdown():
            try:
                self.laser_scan = rospy.wait_for_message("/vector/laser", Range, timeout=1.0)
                rospy.logdebug("Current /vector/laser READY=>")

            except:
                rospy.logerr("Current /vector/laser not ready yet, retrying for getting laser_scan")
    ...
    def move_base(self, linear_speed, angular_speed, epsilon=0.05, update_rate=10):
        ...
        rospy.logdebug("AnkiVector Base Twist Cmd>>" + str(cmd_vel_value))
        self._check_publishers_connection()
